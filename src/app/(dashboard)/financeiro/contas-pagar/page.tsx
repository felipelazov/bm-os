...
import { Plus, Filter, Loader2, Trash2, Pencil, ArrowUpDown, ArrowDownCircle } from 'lucide-react';
...
const statusConfig: Record<TransactionStatus, { label: string; color: string }> = {
  pending: { label: 'Pendente', color: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-400' },
  ...
  overdue: { label: 'Vencido', color: 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400' },
...
export default function ContasPagarPage() {
  ...
  const suggestDreCategory = useCallback((description: string) => {
    ...
    const result = classifyTransaction(
      { date: '', description, value: 0, type: 'expense', document_number: null, raw_data: '' },
      [],
      dreCategories
    );
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Erro ao editar conta');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400">Gerencie suas despesas e pagamentos</p>
        </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950 p-6">
          <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">Nova Conta a Pagar</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Obs</label>
              <input
                type="text"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 p-4">
          <p className="text-sm text-yellow-700 dark:text-yellow-400">A Vencer</p>
        ...
        <div className="rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950 p-4">
          ...
          <p className="mt-1 text-xl font-bold text-green-800 dark:text-green-300">{formatCurrency(totalPaid)}</p>
        </div>
      </div>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="p-0">
                  <EmptyState
                    ...
                    description="Adicione sua primeira conta a pagar"
              ...
              paginatedTransactions.map((t) => (
                <tr key={t.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-4 py-3 text-right text-sm font-medium tabular-nums text-red-600 dark:text-red-400">
